...
def extract_values_ros(values):
  ...
  ret_line = "np_.param(\""+tmp_values[0]+"\", component_config_."+tmp_values[0]+", ("+tmp_values[1]+")"+tmp_values[2]+");"
  return ret_line
...
def extract_values_cfg(values):
  ...
  ret_line = "gen.add(\""+tmp_values[0]+"\", "+tmp_values[1]+"_t, 0, \"Autogenerated parameter based on other files.\", "+tmp_values[2]+line_end+")"
        ...
        for line in in_file:
          for old_var, new_var in in_zip_names:
            ...
            if old_var in line and old_var != new_var:
              line = line.replace(old_var, new_var)           # if line contains an old variable name -> replace it
              occurrences += 1
            elif old_camel_var in line:
              line = line.replace(old_camel_var, new_var)     # if line contains an old camel case variable name -> replace it
              occurrences += 1
          out_file.write(line)
...
regex_values_cfg = re.compile(r'gen.add\("([a-z_A-Z0-9]+)", ?([a-z]+)_t, ?[0-9], ?"[^"]+", ?([0-9\.]*|[a-zA-Z]*|"[^"\']*"),? ?([\-0-9\.]+)?,? ?([\-0-9\.]+)?\)')
      ...
      for line in old:
        if current_file_id > 0 or any(param_name in line for param_name in new_names):
          ...
          if len(params_in_line) > 0:
            ...
            extracted_values[current_file_id][-1][0] = camel_to_snake_word(extracted_values[current_file_id][-1][0])
